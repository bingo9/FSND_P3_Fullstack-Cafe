...
AUTH0_DOMAIN = 'fsnd-ja.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'fullstack-cafe'
...
def get_token_auth_header():
    if 'Authorization' in request.headers:
        ...
        if auth_header:
            any_b_token = auth_header.split(' ')
            if any_b_token[0] and any_b_token[0].lower() == \
               'bearer' and any_b_token[1]:
                return any_b_token[1]
...
@TODO implement @requires_auth(permission) decorator method
    ...
    and check the requested permission return the decorator which passes
